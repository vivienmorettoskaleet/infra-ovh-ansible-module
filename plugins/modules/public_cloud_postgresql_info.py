...
DOCUMENTATION = '''
---
module: public_cloud_postgresql_info
short_description: Get details of OVHcloud Managed PostgreSQL clusters
description:
...
options:
  service_name:
    ...
    type: str
  cluster_id:
    description: PostgreSQL cluster ID (UUID). Mutually exclusive with C(description).
    required: false
    type: str
  description:
...
EXAMPLES = r'''
- name: Get a single PostgreSQL cluster by ID
  synthesio.ovh.public_cloud_postgresql_info:
    service_name: "{{ project_id }}"
    cluster_id: "{{ cluster_id }}"
  register: cluster_info

- name: Get a single PostgreSQL cluster by description
  synthesio.ovh.public_cloud_postgresql_info:
    service_name: "{{ project_id }}"
    description: my-postgresql
  register: cluster_info

- name: Get all PostgreSQL clusters
  synthesio.ovh.public_cloud_postgresql_info:
    service_name: "{{ project_id }}"
...
def run_module():
    ...
    all_clusters = [
        client.wrap_call("GET", f"/cloud/project/{service_name}/database/postgresql/{cid}")
    ...
    if description:
        for cluster in all_clusters:
            if cluster.get("description") == description:
                module.exit_json(changed=False, cluster=cluster)
                return
        module.fail_json(msg=f"No PostgreSQL cluster found with description '{description}'")
