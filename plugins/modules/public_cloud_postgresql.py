...
DOCUMENTATION = '''
---
module: public_cloud_postgresql
short_description: Manage OVHcloud Managed PostgreSQL clusters
description:
  - Create, update, or delete Managed PostgreSQL clusters in an OVHcloud Public Cloud project.
...
EXAMPLES = r'''
- name: Create a single-node PostgreSQL cluster on the public network
  synthesio.ovh.public_cloud_postgresql:
...
- name: Create a PostgreSQL cluster on a private network
  synthesio.ovh.public_cloud_postgresql:
    service_name: "{{ project_id }}"
    description: my-private-postgresql
    plan: business
    version: "16"
    nodes_pattern:
      flavor: db1-15
...
- name: Delete a PostgreSQL cluster
  synthesio.ovh.public_cloud_postgresql:
    service_name: "{{ project_id }}"
    description: my-postgresql
    state: absent
...
def _find_cluster_by_description(client, service_name, description):
    ...
    try:
        cluster_ids = client.wrap_call("GET", f"/cloud/project/{service_name}/database/postgresql")
    ...
    for cluster_id in cluster_ids:
        try:
            cluster = client.wrap_call(
                "GET",
                f"/cloud/project/{service_name}/database/postgresql/{cluster_id}",
            )
        except OVHResourceNotFound:
            continue
...
def run_module():
    ...
    if state == "absent":
        if current is None:
            module.exit_json(changed=False, msg=f"PostgreSQL cluster '{description}' does not exist")

        cluster_id = current["id"]
        if module.check_mode:
            module.exit_json(
                changed=True,
                msg=f"PostgreSQL cluster '{description}' [{cluster_id}] would be deleted (dry run)",
            )
        ...
        module.exit_json(changed=True, msg=f"PostgreSQL cluster '{description}' [{cluster_id}] deleted")
    ...
    if current is None:
        if not module.params.get("plan"):
            module.fail_json(msg="'plan' is required when creating a PostgreSQL cluster")
        if not module.params.get("version"):
            module.fail_json(msg="'version' is required when creating a PostgreSQL cluster")
        if not module.params.get("nodes_pattern") and not module.params.get("nodes_list"):
            module.fail_json(msg="'nodes_pattern' or 'nodes_list' is required when creating a PostgreSQL cluster")

        if module.check_mode:
            module.exit_json(
                changed=True,
                msg=f"PostgreSQL cluster '{description}' would be created (dry run)",
            )
        ...
        result = client.wrap_call(
            "POST",
            f"/cloud/project/{service_name}/database/postgresql",
            **post_kwargs,
        )
        module.exit_json(changed=True, msg=f"PostgreSQL cluster '{description}' created", **result)
    ...
    for param, api_field in immutable_checks:
        ...
        if value is not None and current.get(api_field) != value:
            module.fail_json(msg=f"Cannot change '{param}' for existing PostgreSQL cluster '{description}'")
    ...
    if not needs_update:
        module.exit_json(
            changed=False,
            msg=f"PostgreSQL cluster '{description}' [{cluster_id}] is already up to date",
            **current,
        )
    ...
    if module.check_mode:
        module.exit_json(
            changed=True,
            msg=f"PostgreSQL cluster '{description}' [{cluster_id}] would be updated (dry run)",
        )
    ...
    module.exit_json(
        changed=True,
        msg=f"PostgreSQL cluster '{description}' [{cluster_id}] updated",
        **updated,
    )
